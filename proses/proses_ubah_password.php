...
if(!empty($_POST['ubah_password_validate'])){
    $query = mysqli_query($conn, "SELECT * FROM tb_user WHERE username = '$_SESSION[username_warung]' && password = '$passwordlama'");
    $hasil = mysqli_fetch_array($query);
    if($hasil){
        if($passwordbaru == $repasswordbaru){
            $query = mysqli_query($conn, "UPDATE tb_user SET password='$passwordbaru' WHERE username = '$_SESSION[username_warung]'");
            if($query) {
                $message = '<script>alert("Password Berhasil Diubah");
                            window.history.back()</script>
                            </script>';
            }else{
                $message = '<scrip>alert("Password Gagal Diubah");
                            window.history.back()</script>
                            </script>';
            }
        }else{
